            ...
            <div class="col-sm-12">
                <div class="card mb-4 mt-5">
                    <div class="card-header">
                        ...
                        <span><a href="{{route('createNewMenLifestyle')}}" class="btn btn-primary btn-sm float-end mr-3">ADD PRODUCT</a></span>
                        @endif
                        @endauth
                    </div>
                    <div class="card-body">
                        @if(count($men_lifestyles)>0)
                            <table id="datatablesSimple">
                                ...
                                @foreach($men_lifestyles as $men_lifestyle)
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$men_lifestyle->productName}}</td>
                                        <td>{{$men_lifestyle->productSize}}</td>
                                        <td>{{$men_lifestyle->productDescription}}</td>
                                        <td>
                                            <img src="/storage/men_lifestyles_images/{{$men_lifestyle->productImage}}" alt="" style="height: 100px" class="rounded mx-auto d-block" data-bs-toggle="modal" data-bs-target="#myModal-{{$men_lifestyle->id}}">
  
                                            <!-- Modal -->
                                            <div class="modal fade" id="myModal-{{$men_lifestyle->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-5" id="exampleModalLabel">{{$men_lifestyle->productName}}</h1>
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="container-fluid d-flex justify-content-center">
                                                                <div class="text-center">
                                                                    <img src="/storage/men_lifestyles_images/{{$men_lifestyle->productImage}}" class="rounded" alt="..." style="width: 200px">
                                                                    <br>
                                                                    <h3 class="text-warning">price:  {{$men_lifestyle->productPrice}}</h3>
                                                                    @if(Auth::user())
                                                                    <span><input type="button" value="Add to cart" class="btn btn-primary btn-sm" onclick="addToCart('{{$men_lifestyle->productName}}')"></span>
                                                                    @else 
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            @auth
                                                            @if(Auth::user()->is_Admin==1)
                                                            <a href="{{route('editMenLifestyle', $men_lifestyle->id)}}" class="btn btn-primary btn-sm">Edit</a>
                                                            <a href="{{route('deleteMenLifestyleProduct', $men_lifestyle->id)}}" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this product?');">Delete</a>
                                                            @endif
                                                            @endauth
                                                        </div>
                                                    </div>
