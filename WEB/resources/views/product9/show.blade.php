            ...
            <div class="col-sm-12">
                <div class="card mb-4 mt-5">
                    <div class="card-header">
                        ...
                        <span><a href="{{route('createChildrenShoes')}}" class="btn btn-primary btn-sm float-end mr-3">ADD PRODUCT</a></span>
                        @endif
                        @endauth
                    </div>
                    <div class="card-body">
                        @if(count($children_shoes)>0)
                            <table id="datatablesSimple">
                                ...
                                @foreach($children_shoes as $children_shoe)
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$children_shoe->productName}}</td>
                                        <td>{{$children_shoe->productSize}}</td>
                                        <td>{{$children_shoe->productDescription}}</td>
                                        <td>
                                            <img src="/storage/children_shoes_images/{{$children_shoe->productImage}}" alt="" style="height: 100px" class="rounded mx-auto d-block" data-bs-toggle="modal" data-bs-target="#myModal-{{$children_shoe->id}}">
  
                                            <!-- Modal -->
                                            <div class="modal fade" id="myModal-{{$children_shoe->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-5" id="exampleModalLabel">{{$children_shoe->productName}}</h1>
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="container-fluid d-flex justify-content-center">
                                                                <div class="text-center">
                                                                    <img src="/storage/children_shoes_images/{{$children_shoe->productImage}}" class="rounded" alt="..." style="width: 200px">
                                                                    <br>
                                                                    <h3 class="text-warning">price:  {{$children_shoe->productPrice}}</h3>
                                                                    @if(Auth::user())
                                                                    <span><input type="button" value="Add to cart" class="btn btn-primary btn-sm" onclick="addToCart('{{$children_shoe->productName}}')"></span>
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            @auth
                                                            @if(Auth::user()->is_Admin==1)
                                                            <a href="{{route('editChildrenShoes', $children_shoe->id)}}" class="btn btn-primary btn-sm">Edit</a>
                                                            <a href="{{route('deleteChildrenShoes', $children_shoe->id)}}" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this product?');">Delete</a>
                                                            @endif
                                                            @endauth
                                                        </div>
                                                    </div>
