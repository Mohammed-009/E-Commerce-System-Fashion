                ...
                <div class="col-sm-12">
                    <div class="card mb-4 mt-5">
                        <div class="card-header">
                            ...
                            <span><a href="{{route('createNewWomenOfficial')}}" class="btn btn-primary btn-sm float-end mr-3">ADD PRODUCT</a></span>
                            @endif
                            @endauth
                        </div>
                        <div class="card-body">
                            @if(count($women_officials)>0)
                                <table id="datatablesSimple">
                                    ...
                                    @foreach($women_officials as $women_official)
                                            <tr>
                                                <td>{{$loop->iteration}}</td>
                                                <td>{{$women_official->productName}}</td>
                                                <td>{{$women_official->productSize}}</td>
                                                <td>{{$women_official->productDescription}}</td>
                                                <td>
                                                    <img src="/storage/women_officials_images/{{$women_official->productImage}}" alt="" style="height: 100px" class="rounded mx-auto d-block" data-bs-toggle="modal" data-bs-target="#myModal-{{$women_official->id}}">
          
                                                    <!-- Modal -->
                                                    <div class="modal fade" id="myModal-{{$women_official->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h1 class="modal-title fs-5" id="exampleModalLabel">{{$women_official->productName}}</h1>
                                                                ...
                                                                <div class="modal-body">
                                                                    <div class="container-fluid d-flex justify-content-center">
                                                                        <div class="text-center">
                                                                            <img src="/storage/women_officials_images/{{$women_official->productImage}}" class="rounded" alt="..." style="width: 200px">
                                                                            <br>
                                                                            <h3 class="text-warning">price:  {{$women_official->productPrice}}</h3>
                                                                            @if(Auth::user())
                                                                            <span><input type="button" value="Add to cart" class="btn btn-primary btn-sm" onclick="addToCart('{{$women_official->productName}}')"></span>
                                                                            @else 
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    @auth
                                                                    @if(Auth::user()->is_Admin==1)
                                                                    <a href="{{route('editWomenProduct', $women_official->id)}}" class="btn btn-primary btn-sm">Edit</a>
                                                                    <a href="{{route('deleteWomenProduct', $women_official->id)}}" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this product?');">Delete</a>
                                                                    @endif
                                                                    @endauth
                                                                </div>
                                                            </div>
                      ...
                      <div class="container p-4">
                        <div class="row">
                          <div class="col-lg-3 col-md-3">
                            ...
                            <p>
                              This ia s simple description of the business which includes the types of products present and means of payments, and delivery. All customers are fairly served with good customer services from capable staff                            </p>
                          </div>
                          {{--  --}}
