            ...
            <div class="col-sm-12">
                <div class="card mb-4 mt-5">
                    <div class="card-header">
                        ...
                        <span><a href="{{route('createWomenShoes')}}" class="btn btn-primary btn-sm float-end mr-3">ADD PRODUCT</a></span>
                        @endif
                        @endauth
                    </div>
                    <div class="card-body">
                        @if(count($women_shoes)>0)
                            <table id="datatablesSimple">
                                ...
                                @foreach($women_shoes as $women_shoe)
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$women_shoe->productName}}</td>
                                        <td>{{$women_shoe->productSize}}</td>
                                        <td>{{$women_shoe->productDescription}}</td>
                                        <td>
                                            <img src="/storage/women_shoes_images/{{$women_shoe->productImage}}" alt="" style="height: 100px" class="rounded mx-auto d-block" data-bs-toggle="modal" data-bs-target="#myModal-{{$women_shoe->id}}">
  
                                            <!-- Modal -->
                                            <div class="modal fade" id="myModal-{{$women_shoe->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-5" id="exampleModalLabel">{{$women_shoe->productName}}</h1>
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="container-fluid d-flex justify-content-center">
                                                                <div class="text-center">
                                                                    <img src="/storage/women_shoes_images/{{$women_shoe->productImage}}" class="rounded" alt="..." style="width: 200px">
                                                                    <br>
                                                                    <h3 class="text-warning">price:  {{$women_shoe->productPrice}}</h3>
                                                                    @if(Auth::user())
                                                                    <span><input type="button" value="Add to cart" class="btn btn-primary btn-sm" onclick="addToCart('{{$women_shoe->productName}}')"></span>
                                                                    @else 
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            @auth
                                                            @if(Auth::user()->is_Admin==1)
                                                            <a href="{{route('editWomenShoes', $women_shoe->id)}}" class="btn btn-primary btn-sm">Edit</a>
                                                            <a href="{{route('deleteWomenShoes', $women_shoe->id)}}" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this product?');">Delete</a>
                                                            @endif
                                                            @endauth
                                                        </div>
                                                    </div>
