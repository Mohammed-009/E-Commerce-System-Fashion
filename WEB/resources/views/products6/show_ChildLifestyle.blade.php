            ...
            <div class="col-sm-12">
                <div class="card mb-4 mt-5">
                    <div class="card-header">
                        ...
                        <span><a href="{{route('createChildLifestyle')}}" class="btn btn-primary btn-sm float-end mr-3">ADD PRODUCT</a></span>
                        @endif
                        @endauth
                    </div>
                    <div class="card-body">
                        @if(count($child_lifestyles)>0)
                            <table id="datatablesSimple">
                                ...
                                @foreach($child_lifestyles as $child_lifestyle)
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$child_lifestyle->productName}}</td>
                                        <td>{{$child_lifestyle->productSize}}</td>
                                        <td>{{$child_lifestyle->productDescription}}</td>
                                        <td>
                                            <img src="/storage/children_lifestyles_images/{{$child_lifestyle->productImage}}" alt="" style="height: 100px" class="rounded mx-auto d-block" data-bs-toggle="modal" data-bs-target="#myModal-{{$child_lifestyle->id}}">
  
                                            <!-- Modal -->
                                            <div class="modal fade" id="myModal-{{$child_lifestyle->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-5" id="exampleModalLabel">{{$child_lifestyle->productName}}</h1>
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="container-fluid d-flex justify-content-center">
                                                                <div class="text-center">
                                                                    <img src="/storage/children_lifestyles_images/{{$child_lifestyle->productImage}}" class="rounded" alt="..." style="width: 200px">
                                                                    <br>
                                                                    <h3 class="text-warning">price:  {{$child_lifestyle->productPrice}}</h3>
                                                                    @if(Auth::user())
                                                                    <span><input type="button" value="Add to cart" class="btn btn-primary btn-sm" onclick="addToCart('{{$child_lifestyle->productName}}')"></span>
                                                                    @else 
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            @auth
                                                            @if(Auth::user()->is_Admin==1)
                                                            <a href="{{route('editChildLifestyle', $child_lifestyle->id)}}" class="btn btn-primary btn-sm">Edit</a>
                                                            <a href="{{route('deleteChildLifestyle', $child_lifestyle->id)}}" class="btn btn-danger btn-sm" onclick="return confirm('Do you want to delete this product?');">Delete</a>
                                                            @endif
                                                            @endauth
                                                        </div>
                                                    </div>
